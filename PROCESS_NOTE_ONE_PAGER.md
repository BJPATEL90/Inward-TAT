...
All KPIs use continuous elapsed time and a simple average across the same `COMPLETE` record cohort. Pending and exception records are excluded. Therefore, **KPI1 = KPI2 + KPI3** at both record and aggregate level.
...
6. Only `COMPLETE` records are used for all KPI averages; missing or negative milestones remain pending or move to exceptions.
