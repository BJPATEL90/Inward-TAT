  ...
  return <div className="delay-page">
    ...
    {data && result && <>
      ...
      {view === "summary" && <>
        ...
        <section className="delay-panel"><h2>Data coverage</h2><p>{data.master.rows} master rows · {data.master.uniqueSkus} unique SKUs · {data.master.missingPackRows} missing/invalid pack sizes · {data.master.conflictingPackSkus} SKUs with conflicting packs · {data.master.unclassifiedSkus} unclassified SKUs.</p><p>{result.rows.length - result.eligible.length} selected records lack eligible R-bin completion data. {result.eligible.filter((r) => r.boxes === null).length} eligible records lack a usable box count. Non-R quantities remain visible in each record's detail.</p></section>
      </>}
